...
function toPartialLenses(cmd) {
  if (cmd === profunctor.dictNeglectSum.wanderTag) {
    return pl.elems;
  } else {
    // not really
    return cmd;
...
Optic.prototype.cheatSumOf = function (v) {
  var pllens;
  if (!this.plSumOf) {
    pllens = this.run(profunctor.dictNeglectSum, []).reverse().map(toPartialLenses);
    this.plSumOf = pllens;
  } else {
    pllens = this.plSumOf;
  }

  return pl.sum(pllens, v);
};

Optic.prototype.compileSumOf = function () {
...
Optic.prototype.sumOf = function (v) {
  ...
  // lookup tags
  var wanderTag = profunctor.dictNeglectSum.wanderTag;
  var firstTag = profunctor.dictNeglectSum.firstTag;
  ...
  function process(cmdIdx, value) {
    if (cmdIdx < commands.length) {
      ...
      if (cmd === wanderTag) {
        ...
        process(cmdIdx + 1, value[cmd]);

      } else if (u.isFunction(cmd)) {
        process(cmdIdx + 1, cmd(value));

      } else if (cmd === firstTag) {
        process(cmdIdx + 1, value[0]);
